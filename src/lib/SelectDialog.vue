<template>
  <div>
		<div class="address-ui">
			<div class="item-box">
				<el-tag v-for="(item,index) in checkDatas" :key="index" size="small" closable  @close="removeItem(item)">{{item.label}}</el-tag>
			</div>
			<span class="user-icon">
				<i class="el-input__icon el-icon-s-custom" @click="openAddress"></i>
			</span>
		</div>
...
export default {
  ...
  //   }
  // },
	props:["model","title","datas","checkedDatas","index"],
  data(){
    return {
      searchLeft: '',
      showValue: '',
  ...
  methods: {
    ...
    confirmItems () {
      ...
      this.$emit('getSelectData', this.checkDatas, this.showValue,this.index)
    },
    // 取消选择数据
    closeItems () {
      // 清空选择数据
